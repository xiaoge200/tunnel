...
- `tunnel.rs` — the SSH engine:
  ...
  - `TunnelRuntime` (one per running tunnel): connect + auth (password or `load_secret_key`, 15 s timeout, keepalive 15 s × 3) → spawn one listener task per forward rule → a 1 Hz metrics task computing per-second byte deltas from `AtomicU64` accumulators. Ends on user stop vs SSH-disconnect watcher (`Notify` polling `is_closed()` every 2 s). **Cooperative shutdown**: stop = set flag; tasks poll every ~200 ms.
  - Status publishing has **one funnel**: free fns `publish_metric` / `unregister_runtime` (shared with the manager via the `RunningMap` Arc) update the map → `emit("tunnel-metric")` → `refresh_tray`. 1 Hz metric ticks emit directly (NOT through the funnel, to avoid rebuilding the tray menu every second). A runtime publishes its final status (`Disconnected` if stopped, `Error` if SSH dropped; a stop during connect publishes `Disconnected` too) *before* unregistering; `unregister_runtime` guards against a stale runtime deleting a newer one via `Arc::ptr_eq` on the stopped flag.
...
- Two tunnels running concurrently with the same `local_port`: the second `bind` fails with a log only and its tunnel otherwise keeps running (accepted behavior).
